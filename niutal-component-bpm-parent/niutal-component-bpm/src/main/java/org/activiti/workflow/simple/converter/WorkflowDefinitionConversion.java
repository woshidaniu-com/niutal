...
public class WorkflowDefinitionConversion {
	...
	protected List<UserTask> addedUserTasks;
	...
	protected boolean addUserTaskEnabled = true;
	...
	public WorkflowDefinitionConversion(WorkflowDefinitionConversionFactory factory,
	...
	public void convert() {
		...
		this.addedUserTasks = new LinkedList<UserTask>();
		// Create new process
		...
		if (conversionFactory.getAllWorkflowDefinitionConversionListeners() != null) {
			for (WorkflowDefinitionConversionListener conversionListener : conversionFactory
					.getAllWorkflowDefinitionConversionListeners()) {
	...
	public void convertSteps(List<StepDefinition> stepDefinitions) {
		...
		for (StepDefinition step : stepDefinitions) {
			StepDefinitionConverter stepConverterFor = conversionFactory.getStepConverterFor(step);
			
			if (conversionFactory.getAllWorkflowDefinitionConversionListeners() != null) {
				for (WorkflowDefinitionConversionListener conversionListener : conversionFactory
						.getAllWorkflowDefinitionConversionListeners()) {
					conversionListener.beforeStepConversion(this, stepConverterFor, step);
				}
			}
			
			stepConverterFor.convertStepDefinition(step, this);
			
			if (conversionFactory.getAllWorkflowDefinitionConversionListeners() != null) {
				for (WorkflowDefinitionConversionListener conversionListener : conversionFactory
						.getAllWorkflowDefinitionConversionListeners()) {
					conversionListener.afterStepConversion(this, stepConverterFor, step);
				}
			}
		}
	}
	...
	/**
	 * 
	 * <p>方法说明：add user task<p>
	 * <p>作者：a href="#">xiaokang[1036]<a><p>
	 * <p>时间：2016年12月7日上午9:25:00<p>
	 */
	public void addUserTask(UserTask userTask){
		this.addedUserTasks.add(userTask);
	}

	public boolean isAddUserTaskEnabled() {
		return addUserTaskEnabled;
	}

	public void setAddUserTaskEnabled(boolean addUserTaskEnabled) {
		this.addUserTaskEnabled = addUserTaskEnabled;
	}

	public List<UserTask> getAddedUserTasks() {
		return addedUserTasks;
	}
