...
public class WorkFlowLogViewHtmlCreator {
	...
	private static String HTML_START = "<body><div class='splc1'>";
...
//		sb.append("</div>");
	...
	private void createLogInfo(){
		...
		for(SpAuditingLog log:logList){
			...
			sb.append("<div class='content'><div>审核环节：<span class='"+span_class_1+"'>"+log.getOtype()
					+"</span>审核人：<span class='black1'>"+shr
					+"</span>审核时间：<span class='black1'>"+this.format(log.getLogTime(),timeFormat)
					+"</span></div> <div>审核意见：<span class='"+span_class_2+"'>" +log.getOsuggestion() 
					+"</span></div></div><div style='clear:both'></div></div></div>");
			if(i != logList.size()){
				sb.append("<div class='next'><img src='<%=stylePath %>/images/"+img+"' /></div>");
			}
			i++;
		}
	}
