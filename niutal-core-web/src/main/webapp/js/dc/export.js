...
function saveConfig(){
	...
	alertPrompt('输入设置名称', function(){
		...
		if(data != undefined || jQuery.trim(data) != ''){
			jQuery.post(
					_path+"/niutal/dc/export_saveCustomConfig.html",
					{
...
function deletePh(id,mc,$this){
	alertConfirm("确认要删除 ‘<b>"+mc+"</b>’ 配置信息？",function(){
		jQuery.post(_path+"/niutal/dc/export_deleteCustomConfig.html?_t="+new Date().getTime(),{"exportPHID":id},function(data){
			if (Boolean(data)){
				window.location.reload();
			} else {
				alert("操作失败！");
...
function selectPh(id,$this){
	...
	jQuery.post(_path+"/niutal/dc/export_cxConfigPhZdList.html",
			{"id": id},
			function(data){
