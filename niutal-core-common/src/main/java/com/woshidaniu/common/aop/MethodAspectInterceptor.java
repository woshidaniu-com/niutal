...
/**
 * 
 *@类名称	: MethodAspectInterceptor.java
 *@类描述	：基于Spring AOP 的方法切面拦截器
 *@创建人	：kangzhidong
 *@创建时间	：Mar 21, 2016 9:43:58 AM
 *@修改人	：
 *@修改时间	：
 *@版本号	:v1.0
 */
public class MethodAspectInterceptor extends AbstractAspectInterceptor {
	...
	public void doBefore(JoinPoint point) throws Throwable {
    	...
    	//获得执行方法
		String methodName = BlankUtils.isBlank(before) ? "doBefore" : before.method();
		
		 //查询日志注解
		Comment comment = method.getAnnotation(Comment.class);
		if(!BlankUtils.isBlank(comment)){
		...
		if(StringUtils.hasText(serviceID)&&StringUtils.hasText(methodName)){
			//从spring上下文获得bean
			Object bean = ServiceFactory.getService(serviceID);
			if(bean!=null){
				//获得前置通知调用方法
	    		Method doBefore = BeanUtils.findMethod(bean.getClass(), methodName,Invocation.class);
	    		//执行doBefore
	    		if(doBefore!=null){
        ...
        After after = target.getClass().getAnnotation(After.class);
 		if( after!= null){
        	...
        	if(StringUtils.hasText(serviceID)&&StringUtils.hasText(methodName)){
        		...
        		if(bean!=null){
        			//获得后置通知调用方法
            		Method doAfter = BeanUtils.findMethod(bean.getClass(), methodName,Invocation.class);
            		//执行doAfter
            		if(doAfter!=null){
        ...
        Exceptional exceptional = target.getClass().getAnnotation(Exceptional.class);
 		if(exceptional!=null){
        	...
        	if(StringUtils.hasText(serviceID)&&StringUtils.hasText(methodName)){
        		...
        		if(bean!=null){
        			//获得异常通知调用方法
            		Method doException = BeanUtils.findMethod(bean.getClass(), methodName,Invocation.class);
            		//执行doException
            		if(doException!=null){
        				doException.invoke(bean,new Object[]{ invocation });
            		}else{
