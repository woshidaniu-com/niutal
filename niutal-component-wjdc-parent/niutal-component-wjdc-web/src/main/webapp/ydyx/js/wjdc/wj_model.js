...
WjModel_TiMu.prototype.getTmHtml_Dhwb=function(){
	...
	var tmmc=this.getTmMcHtml_comman();
	var tmxx=jQuery("<div>", {
		"class": "list_exam_con"
	}).append(jQuery("<textarea>", {name: "textname_"+this.stid, id: "textid_"+this.stid, rows: "1", "class": "textarea margin_tb10 w_b90 padding_l padding_r"}));
	return tm.append(tmmc).append(tmxx);
	//var tmMc=this.getTmMcHtml_comman();
...
WjModel_TiMu.prototype.getTmHtml_Dhwbs=function(){
	...
	var tmxx=jQuery("<div>", {
		"class": "list_exam_con"
	}).append(jQuery("<textarea>", {name: "textname_"+this.stid, id: "textid_"+this.stid, rows: "5", "class": "textarea margin_tb10 w_b90 padding_l padding_r"}));
	return tm.append(tmmc).append(tmxx);
	//var tmHtml="";
...
WjModel_TiMu.prototype.getTmHtml_Stdl=function(){
	...
	return jQuery("<h3>", {"class": "title_1", text: this.stmc});
	/*var tmHtml="";
	var e_stmc_css=document.createElement("h3");
...
WjModel_TiMu.prototype.getTmHtml_comman = function(){
	...
	for(var i=0;i<xxs.length;i++){
		var xx=xxs[i];
		var input_val=xx.xxid;
		var input_name="xxname_"+this.stid;
		var input_id=xx.xxid;
		var input_text=xx.xxmc;
		...
		if((this.stlx==WjModel_STLX.DXZH||this.stlx==WjModel_STLX.DXSZH)&&(i==xxs.length-1)){
			jQuery("<input type='text' class='input margin_l' name='textname_"+this.stid+"' id='textid_"+this.stid+"'/>").appendTo(label);
		}
	}
	return tm.append(tmmc).append(tmxx);
}
...
WjModel_TiMu.prototype.getTmMcHtml_comman = function(){
	...
	if(WjModel_Parameter.autoAddStbh){
		return jQuery("<h4>", {"class": "title_2", text: this.stbh+"."+this.stmc});
	}else{
		return jQuery("<h4>", {"class": "title_2", text: this.stmc});
	}
	/*var tmMcHtml="";
...
WjModel_WenJuan.prototype.getWjHtml=function(){
    var wjHtml = jQuery('<div>', {"class": "list_exam"});
    var tms = this.tmxx;
    for (var i = 0; i < tms.length; i++) {
        var tm = tms[i];
        wjHtml.append(tm.getTmHtml());
    }
	return wjHtml;
}
...
WjModel_WenJuan.prototype.getWjSubmitCheckZt=function(ifAlert){
	...
	for(var i=0;i<tms.length;i++){
		...
		if(tm.stlx==WjModel_STLX.DX||tm.stlx==WjModel_STLX.DXZH||tm.stlx==WjModel_STLX.DXS||tm.stlx==WjModel_STLX.DXSZH){
			...
			if(user_select.length>0&&(tm.stlx==WjModel_STLX.DXS||tm.stlx==WjModel_STLX.DXSZH)){
				if(tm.xxkzdxzs!=""&&tm.xxkzdxzs<user_select.length){
					if (ifAlert) {
						adderror("\"" + tm.stmc + "\"最多可选择"+tm.xxkzdxzs+"项！");
					}
					return false;
				}
			}
			...
			if(user_select.length>0&&(tm.stlx==WjModel_STLX.DXZH||tm.stlx==WjModel_STLX.DXSZH)){
				...
				if(jQuery("#textid_"+tm.stid).val().length > 100){
					if (ifAlert) {
						adderror("\"" + tm.stmc + "\"输入字数过多，请限制在100以内！");
					}
					return false;
				}
			}	
		...
		if(tm.stlx==WjModel_STLX.DHWB||tm.stlx==WjModel_STLX.DHWBS){
			...
			if(text.length > 500){
				if (ifAlert) {
					adderror("\""+tm.stmc+"\"输入字数过多，请限制在500以内！");
				}
				return false;
			}
		}
