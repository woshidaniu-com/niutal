...
public class DefaultActivitiEventListener implements ActivitiEventListener {
	...
	public void onEvent(ActivitiEvent event) {
		...
		case JOB_CANCELED:
			...
			break;
		case CUSTOM:
			if (event instanceof ActivitiCustomEvent) {
				ActivitiCustomEvent extendEvent = (ActivitiCustomEvent) event;
				...
				}else if (extendActivitiEvent == ExtendActivitiEventType.REBOOT_AUDITOR_REVOCATION_EVENT) {
					onRebootAuditorRevocation(event);
				...
				}else if(extendActivitiEvent == ExtendActivitiEventType.INITIATOR_SUSPENSION_EVENT){
					onInitiatorSuspension(event);
				}else if(extendActivitiEvent == ExtendActivitiEventType.INITIATOR_ACTIVATION_EVENT){
					onInitiatorActivation(event);
				}else if(extendActivitiEvent == ExtendActivitiEventType.PROCESS_FALLBACK_EVENT){
					onProcessFallback(event);
				}
			}
			break;
		default:
	...
	public void onJobCanceled(ActivitiEvent event) {
		AbstractEventListener businessCallback = BusinessContext.getBusinessCallback();
	...
	public void onRebootAuditorRevocation(ActivitiEvent event) {
		AbstractEventListener businessCallback = BusinessContext.getBusinessCallback();
		if (businessCallback != null) {
			businessCallback.onRebootAuditorRevocation(event);
		} else
			logDEBUG_NOT_IMPLEMENTED(event.getType());
	...
	public void onInitiatorSuspension(ActivitiEvent event){
		AbstractEventListener businessCallback = BusinessContext.getBusinessCallback();
		if (businessCallback != null) {
			businessCallback.onInitiatorSuspension(event);
		} else
			logDEBUG_NOT_IMPLEMENTED(event.getType());
	} 
	...
	public void onInitiatorActivation(ActivitiEvent event){
		AbstractEventListener businessCallback = BusinessContext.getBusinessCallback();
		if (businessCallback != null) {
			businessCallback.onInitiatorActivation(event);
		} else
			logDEBUG_NOT_IMPLEMENTED(event.getType());
	} 
	...
	public void onProcessFallback(ActivitiEvent event) {
		AbstractEventListener businessCallback = BusinessContext.getBusinessCallback();
		if (businessCallback != null) {
			businessCallback.onProcessFallback(event);
		} else
			logDEBUG_NOT_IMPLEMENTED(event.getType());
	}
