...
<html>
	...
	<body>
		...
		<div class="formbox">
			<!--标题start-->
			<h3 class="datetitle_01" id="hpannel">
				<span id="pannel">缓存列表</span>
				<button id="cache_reload_all_trigger" type="button" class="btn btn-sm btn-danger" value="" style="display:none;">刷新全部</button>
			</h3>
			<!--标题end-->
			<table width="100%" class="table table-hover table-bordered text-center" id="cache_table">
				...
				<tbody>
					[#if cacheConfigList?size > 0]
						[#list cacheConfigList as item]
							<tr>
								<td>${item.name}</td>
		...
		<script type="text/javascript">
				jQuery(function(){
					...
					jQuery("#pannel").mouseover(function(){
						jQuery("#hpannel").find("button").show();
					});
					jQuery("#hpannel").mouseout(function(){
						setTimeout(function(){
							jQuery("#hpannel").find("button").hide();
						},5000);
					});
					
					//刷新全部缓存
					jQuery('#cache_reload_all_trigger').click(function(){
						var msg = "";
						jQuery('#cache_table input[name="cache_reload_trigger"]').each(function(i,n){
							var cacheKey = jQuery(n).attr('key');
							if(cacheKey){
								jQuery.post( "refreshCache.zf",
											{"CACHE_KEY":cacheKey},
											function(data){msg += data["message"];},
											"json");
							}
						});
						$.alert("刷新全部缓存成功!")
