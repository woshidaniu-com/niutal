...
jQuery(function($) {
	...
	.off("click touchend", "#updatePassword").on("click touchend", "#updatePassword", function(e) {
		$.showDialog(_path + "/xtgl/mmgl_xgMm.html", $.i18n.niutal["titles"]["pwd"], $.extend({}, addConfig, {
			"width": "600px"
		}));
	})
	// 兼容菜单超出窗口显示问题
	...
	// Ajax动态加载新闻内容
	$("#newsnotice").load(_path + "/xtgl/index_cxNews.html", {}, function() {
		// 滚动条
	...
	// Ajax动态加载用户首页显示信息
	$("#yhxxIndex").load(_path + "/xtgl/index_cxYhxxIndex.html?xt=jw");
