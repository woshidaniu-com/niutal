...
function customExport(width,height,url,exportFun){
	var w = parseInt(width) || 680;
	var h = parseInt(height) || 365;
	
	showDialog('自定义导出配置',w,h,url,
			{button:[{name:'保存设置',callback:function(){
					this.content.saveConfig();
					//if(this.content.doExport()){
					//	exportFun();
					//}
					return false;
...
//关闭帮助提示信息
function closeHelpTitle(obj){
	jQuery(obj.parentNode).fadeOut('normal');
}
//打开帮助提示信息
function openHelpTitle(){
	jQuery(".prompt").fadeToggle("normal");
}