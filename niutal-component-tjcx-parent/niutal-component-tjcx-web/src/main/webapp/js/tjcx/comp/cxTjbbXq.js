...
function setInit() {
	...
	jQuery.ajax( {
		type : "post",
		// async: false,
		url : _path + "/zfxg/tjcx/tjbbXq_cxTjbbxqBt.html?doType=query&timestamp="
				+ new Date().getTime(),
		data : {
			xmdm : xmdm
		},
...
function tjcxsj() {
	...
	var url = _path + '/zfxg/tjcx/tjbbXq_cxTjbbXqsj.html?doType=query';
...
function setDcButton(){
	...
	if(btn_dc != null){
		btn_dc.click(function () {
			...
			var url = _path+'/zfxg/tjcx/tjcx_export.html';
			url += "?timestamp=" + new Date().getTime();

		//	jQuery("#form1").attr("action",url);
		//	jQuery("#form1").submit();
			customExport(680,500,_path+'/zfxg/tjcx/tjcx_customExport.html?xmdm=' + xmdm,function(){
				jQuery("#form1").attr("action",url);
				jQuery("#form1").submit();
			});
		});
