...
function onShow() {
	...
	jQuery.ajax( {
		type : "post",
		url : _path + "/zfxg/tjcx/tjbb_tjsj.html?timestamp=" + new Date().getTime(),
		data : {
			xmdm : xmdm,
			gltj : gltj,
			oldGltj : oldGltj,
		...
		success : function(data) {
			...
			tjbbModel = data;
			if (tjbbModel == null) {
				showAlertDivLayer('统计选项不合法，请重新设置统计项！','',{'clkFun':function(){
					window.close();
				 }
				});
				return false;
...
function tjbbXqWin(){
	...
	var url = _path + "/zfxg/tjcx/tjbbXq_cxTjbbXq.html?timestamp=" + new Date().getTime();
...
function createTable() {
	var sHtml = "";
	sHtml += "<table  cellSpacing=0 cellPadding=2  align='center' border='1'>";
...
function setButton() {
	...
	if(btn_cx != null){
		btn_cx.click(function() {
			var title = "使用说明";
			var url = _path + "/zfxg/tjcx/tjbb_tjlbBzsm.html";
			showDialog(title, 1024, 580,url);
		});
	}
