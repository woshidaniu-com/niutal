jQuery(function($){
	...
	var options = {
			 ...
			 columns: [
	            ...
	            {field: 'name',title: ' 流 程 名 称 ',sortable:false,width:'100px',align:'center'},
//	            {field: 'category', title: ' 设 计 器 类 别 ',sortable:false,width:'80px',align:'center',formatter:function(value,row,index){
//	            	var ret;
//	            	if(value == 'simple'){
...
//	            }}, 
						...
						callback : function() {
							...
							submitForm("ajaxForm",function(responseText,statusText){
								...
								if(responseText["status"] == "success"){
									...
									if(editor == 'simple'){
										$.showDialog(editorUrl,'编辑模型[简单]',$.extend({},addConfig,{"width":"900px",buttons:{
											success : {
												label : "保存",
												className : "btn-primary",
												callback : function() {
									    	...
									    	var editorUrl = _path+"/modeler.html?modelId="+modelId+"&context="+_path;
										window.open(editorUrl);
										//$.showDialog(editorUrl,'编辑模型[高级]',$.extend({},viewConfig,{"width":($(window).width()*0.9)}));
									}
	...
	jQuery("#btn_ck").click(function () {
		...
		if(rowData['category'] == 'simple' ){
			...
			$.showDialog(editorUrl,'查看模型[简版]',$.extend({},viewConfig,{"width":'900px'}));
	...
	jQuery("#btn_xg").click(function () {
		...
		if(rowData['category'] == 'simple' ){
			...
			var editorUrl = _path+"/modeler.html?modelId="+ids[0]+"&context="+_path;
			window.open(editorUrl);
		}
	});	
	...
	jQuery("#btn_dr").click(function () {
		var url = _path + "/processManagement/moduler/upload.zf";
	...
	jQuery("#btn_dc").click(function () {
		...
		var exportModelForm_url =  _path + "/processManagement/moduler/" + ids[0] + "/export.zf";
