$(function() {
			...
			function(data) {
				if ($.founded(data['model'])) {
					...
					if ($.founded(model['description'])) {
						$('#ajaxForm #description').text(
							model['description']);
					}
					if ($.founded(model['category'])) {
						$('#ajaxForm #category').val(
							model['category']);
					}
				// 设置task列表
				...
				if ($.founded(taskDefinitions)) {
					$(
						'#process_defination_task_tab tbody tr:first')
					.remove();
					$
					.each(
						taskDefinitions,
						function(i, n) {
							var taskRow = createProcessTaskDefinationRow(process_defination_task_tab_row
								.clone());
							taskRow
							.find(
								"input[name='taskName']")
							...
							.each(
								...
								function(
									...
									.append(
										createProcessDefinationTaskCandidateUserEl(
											m['userId'],
											m['userName']));
								});
						...
						.each(
							...
							function(
								...
								.append(
									createProcessDefinationTaskCandidateGroupEl(
										m['groupId'],
										m['groupName']));
							});
					}
					$(
						'#process_defination_task_tab tbody')
					.append(taskRow);
				});
	...
	function createProcessDefinationTaskCandidateUserEl(id, name) {
		...
		var hidden = $('<input type="hidden" name="candidateUsers" value="'
			+ id + '|' + name + '">');
		el
		.append('<button class="btn btn-default btn-sm dropdown-toggle" type="button" data-toggle="dropdown">'
	...
	function createProcessDefinationTaskCandidateGroupEl(id, name) {
		var el = $('<div>').addClass('btn-group').prop("title", name);
		var hidden = $('<input type="hidden" name="candidateGroups" value="'
			+ id + '|' + name + '">');
		el
		.append('<button class="btn btn-default btn-sm dropdown-toggle" type="button" data-toggle="dropdown">'
     ...
     */
     		...
     		$('#candidate_users_choose_btn', row).click(
     			function() {
     				$.showDialog(_path + "/processUser/list.zf", '流程用户', $
     					.extend({
     						'candidate_users_container' : $(
     							'#candidate_users_container', row)
     					}, processUserChooserConfig));
     			});

     		$('#candidate_groups_choose_btn', row).click(
     			function() {
     				$.showDialog(_path + "/processGroup/list.zf", '流程用户组',
     					$.extend({
     						'candidate_groups_container' : $(
     							'#candidate_groups_container', row)
     					}, processGroupChooserConfig));
     			});
     		return row;
     	}
     ...
     $(document).off('click', '.candidate_name').on(
     	'click',
     	'.candidate_name',
     	function() {

     		$(this).parent().find('#candidate_users_container')
     		.toggleClass('hide');
     		$(this).parent().find('#candidate_groups_container')
     		.toggleClass('hide');
