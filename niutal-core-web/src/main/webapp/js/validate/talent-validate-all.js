...
zfValidate.i18Phone = "合法的格式形如：13265895632";
...
zfValidate.VF = zfValidate.C.ext({
	init : function() {
		...
		this.postcode = new zfValidate.RV().set(/^[0-9]\d{5}(?!\d)$/).setI18(zfValidate.i18Postcode);
		this.tel = new zfValidate.RV().set(/^[+]{0,1}(\d){1,3}[ ]?([-]?((\d)|[ ]){1,12})+$/).setI18(zfValidate.i18Tel);
		this.phone = new zfValidate.RV().set(/^1\d{10}$/).setI18(zfValidate.i18Phone);
...
zfValidate.ZZJGDM = zfValidate.BV.ext({
	   v:function(trimedValue, indexOfElements, elements, field){
			if(trimedValue.length != 9){
				return false;
			}
			var strP= /^[0-9]*[1-9][0-9]*$/;
			if(!strP.test(trimedValue.substring(0,8))){
				return false;
			}
			var charArr = trimedValue.split("");
			var total = charArr[0]*3+charArr[1]*7+charArr[2]*9+charArr[3]*10+
			charArr[4]*5+charArr[5]*8+charArr[6]*4+charArr[7]*2;
			var checkCode = 11 - total%11;
			if(checkCode == 10){
				checkCode = "X";
			}else if(checkCode == 11){
				checkCode = "0";
			}
			if(checkCode != charArr[8]){
				return false;
			}
			return true;
		},
