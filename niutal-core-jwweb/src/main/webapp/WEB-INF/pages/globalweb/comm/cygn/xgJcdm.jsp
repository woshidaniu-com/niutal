...
<s:form name="ajaxForm" id="ajaxForm" cssClass="form-horizontal sl_all_form"  action="/cygn/jcdm_xgBcJcdm.html" method="post" theme="simple">
				...
				<div class="form-group" <s:if test="desc != null">style="margin-bottom: 5px;"</s:if>>
					...
					<div class="col-sm-8">
						<input type="text" id="<s:property value="name"/>" name="<s:property value="name"/>" 
			       		...
			       		class="form-control Wdate" value="<s:property value="value"/>" readonly="readonly"/>
			       		 <s:if test="desc != null">
			       			<p class="form-control-static smaller-80 red" style="padding-top: 2px;padding-bottom: 2px;">注：<s:property value="desc"/></span>
			       		 </s:if>
					</div>
				...
				<div class="form-group" <s:if test="desc != null">style="margin-bottom: 5px;"</s:if>>
					...
					<div class="col-sm-8">
						<textarea id="<s:property value="name"/>" name="<s:property value="name"/>"
		       		 	<s:if test="validateMap!=''">
				       		  	<%--validate="<s:property value="validateMap"/>" 修改是textarea框的提示语句--%>
				       	</s:if>
						...
						<select id="<s:property value="name"/>" name="<s:property value="name"/>"  class="form-control chosen-select"
						  ...
						  >
		       		 		...
		       		 		<s:iterator value="#col.selectList" id="sub" status="st">
		       		 			<s:if test="value==#sub.MC">
		       		 				<option value="<s:property value="#sub.ID"/>"  selected="selected"><s:property value="#sub.MC"/></option>
		       		 			</s:if>
		       		 			<s:else>
	...
	jQuery(function($) {
		$("#ajaxForm").validateForm({
			beforeValidated:function(){
				...
				var isValidate = true;
				jQuery.ajax({
					url:_path+'/cygn/jcdm_cxUniqueValidate.html?doType=update',
					async: false,
					type:"post",
					dataType:"json",
