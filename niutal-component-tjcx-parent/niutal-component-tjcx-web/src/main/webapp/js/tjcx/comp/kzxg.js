...
jQuery(function() {
	...
	KzxgGrid = Class.create(BaseJqGrid,{  
		...
		rowNum : 10, // 每页显示记录数
	    url: _path+'/zfxg/tjcx/kzsz_kzcx.html', //这是Action的请求地址  
	    colModel:[
		...
		sortname: 'szmc', //首次加载要进行排序的字段 
	 	sortorder: "",
	 	viewrecords: true,
	 	editurl: _path+'/zfxg/tjcx/kzsz_operation.html'
	});
...
function bdan(){
	...
	if(btn_xg!=null){
		btn_xg.click(function () {
			...
			var url = _path+'/zfxg/tjcx/kzsz_operation.html'
			var url=_path+"/zfxg/tjcx/kzsz_kzszUpdate.html?kzszid=" + row['kzszid']; 
	...
	if(btn_sc!=null){
		btn_sc.click(function () {
			...
			if (ids.length == 0){
				...
				var url = _path+"/zfxg/tjcx/kzsz_kzszDelete.html";
				var _do = function(){
