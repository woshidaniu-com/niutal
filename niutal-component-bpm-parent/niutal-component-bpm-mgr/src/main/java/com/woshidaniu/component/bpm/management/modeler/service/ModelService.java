...
import org.activiti.engine.extend.service.impl.StencilServiceImpl;
...
public class ModelService implements ModelDataJsonConstants {
	...
	@Autowired
	protected StencilServiceImpl stencilService;
	 ...
	 * <p>
	 */
	public 	ObjectNode addModuler(String modelName, String modelDesc, String modelEditor, String bizId) throws Exception {
		Model modelData = repositoryService.newModel();
		if (ProcessModelDataJsonConstants.MODEL_CATEGORY.equals(modelEditor) || "advanced".equals(modelEditor)) {
			...
			//"process_id":"process111222222","name":"","documentation":""
			propertiesNode.put("process_id", "pid-"+UniqID.getInstance().getUniqIDHash());
			propertiesNode.put("name", modelName);
			propertiesNode.put("documentation", modelDesc);
			//加入事件监听属性
			propertiesNode.put("eventlisteners", stencilService.createProcessEventListenerNode());
			//加入业务组ID
			propertiesNode.put("bizId", bizId == null ? "" : bizId);
			...
			//加入业务组ID
			editorNode.put("bizId", bizId == null ? "" : bizId);
			repositoryService.addModelEditorSource(modelData.getId(), editorNode.toString().getBytes("utf-8"));
		}
