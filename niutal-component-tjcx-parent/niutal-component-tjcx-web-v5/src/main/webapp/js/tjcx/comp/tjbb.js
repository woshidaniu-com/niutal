...
function setInitCxkz(obj){
	if(obj != null){
		...
		kzzsTjx(obj.tsx);
		//jQuery("#form1").find("[name='kzszid']").val(obj.kzszid);
	}
}
...
function createBblHtml() {
	...
	sHtml += "<div class='panel-body'>";//列的展示
	sHtml += "<ul class='tag-list' id='demo0'>";
	sHtml += "</ul>";
	sHtml += "<div class='clearfix'></div>";
	sHtml += "</div>";

	sHtml += "<div class='panel-footer'>";//已选横向列
	sHtml += "<span class='float_l padding-t10 fwhite'>已选横向列 </span>";
	sHtml += "<ul class='tag-list tag-list1' id='demo1'>";
	sHtml += "</ul>";
	sHtml += "<div class='clearfix'></div>";
	sHtml += "</div>";

	sHtml += "<div class='panel-footer'>";//已选纵向列
	sHtml += "<span class='float_l padding-t10 fwhite'>已选纵向列 </span>";
	sHtml += "<ul class='tag-list tag-list1' id='demo2'>";
	sHtml += "</ul>";
	sHtml += "<div class='clearfix'></div>";
	sHtml += "</div>";
	return sHtml;
...
function saveOrder() {
	...
	if(curUlId == "demo0"){//从已选列拖到待选列去掉叉号
		jQuery(this).find(".fa-trash-o").remove();
	}else{//从未选列拖到已选列，增加叉号
		jQuery(this).not(":has('i')").find("span").append(yxbblYcHtml());
	}
...
function yxbblYc(obj){
	jQuery("#demo0").append(jQuery(obj).parents("li"));
	jQuery("#demo0 li").find("i").remove();//删除叉号
}
...
//	return " <i class='glyphicon glyphicon-trash' onclick='yxbblYc(this)'></i>";
	return "";
}
		...
		for ( var i = 0; i < bbhxlArr.length; i++) {
			jQuery("#demo0 li").each(function(index){//默认显示报表列
				...
				if(bbhxlArr[i] == bblzddm){
					jQuery(this).find("span").append(yxbblYcHtml());
					jQuery("#demo1").append("<li>" + jQuery(this).html() +  "</li>");
					jQuery(this).remove();
				}
			});
		}
		...
		for ( var i = 0; i < bbzxlArr.length; i++) {
			jQuery("#demo0 li").each(function(index){//默认显示报表列
				...
				if(bbzxlArr[i] == bblzddm){
					jQuery(this).find("span").append(yxbblYcHtml());
					jQuery("#demo2").append("<li>" + jQuery(this).html() +  "</li>");
					jQuery(this).remove();
				}
			});
		}
...
function kzzsTjx(tjx){
	...
	jQuery("#tjxDiv input:checkbox:not([name='zs'])").removeProp("checked");
	var arr = tjx.split(",");
	jQuery.each(arr,function(index,value){
		jQuery("#tjxDiv input:checkbox[value='"+value+"']").prop("checked",true);
	});	
}
...
function checkTjlb(){
	...
	if(jQuery("#demo2 [name='bblzddm']").length > 3){
		$.alert("报表纵向列至多为3项，请重新选择(一次展示过多的统计项，会影响统计效率，请慎重)！");
		return false;		
	}
	...
	if(tsxList == null || tsxList.length == 0){//不存在数据项
		...
		if(sjxmsPTGLength == 1 && (sjxmsSUMLength == 0 || hjxZXLHJLength == 0)){
			$.alert("数据项【百分比】模式已设置，合计项【纵向合计】和数据项模式【总和】必须同时设置！");
			return false;	
		}
