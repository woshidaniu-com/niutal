...
activitiModeler.config(['$selectProvider', '$translateProvider', function ($selectProvider, $translateProvider) {
      ...
      angular.extend($selectProvider.defaults, {
        ...
        function($rootScope, $timeout, $modal, $translate, $location, $window, $http, $q) {
            ...
            function fetchModel(modelId) {
                ...
                $http({method: 'GET', url: modelUrl}).
                    success(function (data, status, headers, config) {
                       ...
                       if($rootScope.modelData.model.properties && $rootScope.modelData.model.properties['bizId']){
                	   fetchBizField($rootScope.modelData.model.properties['bizId']);
                       }else{
            ...
            /*Helper method to fetch bizFiled data from server*/
            function fetchBizField(bizId){
        	var bizFieldsUrl = KISBPM.URL.getBizFiled(bizId);
        	$http({method: 'GET', url: bizFieldsUrl}).
                success(function (data, status, headers, config) {
                    $rootScope.bizFieldData = angular.fromJson(data);
                    $rootScope.editorFactory.resolve();
                }).
                error(function (data, status, headers, config) {
                  //console.log('Error loading bizField with id ' + bizId + ' ' + data);
                });
            }
            ...
            $rootScope.$on('$includeContentLoaded', function (event) {
	        	...
	        	$rootScope.holdOn('open');
	        	
	        	$rootScope.Log = ORYX.Log;
