...
public class DefaultBPMServiceImpl implements BPMService, InitializingBean {
	...
	protected void preCheck(String processDefinationKey, String processStarter, String bizId) {
		BPMUtils.assertProcessDefinationKeyNotNULL(processDefinationKey);
		BPMUtils.assertProcesssInstanceStarterNotNULL(processStarter);
		BPMUtils.assertBusinessKeyNotNULL(bizId);
	}

	protected void preSetup(String authid, BPMEventListener eventListener) {
		BPMUtils.setupAuthId(authid);
		if (eventListener != null) {
			BusinessContext.setBusinessCallback(eventListener);
		}
	}
	...
	protected void preSetup(Decision decisionInfo){
		if(decisionInfo != null){
			BusinessContext.setDecisionInfo(decisionInfo);
		}
	}

	protected void postSetup() {
		BPMUtils.setupAuthId(null);
		BusinessContext.removeBusinessCallback();
		BusinessContext.removeDecisionInfo();
	}
	...
	@Override
	public ProcessInstance startProcess(String processDefinationKey, String userId, String bizId,
			Map<String, Object> processVariables, BPMEventListener eventListener) throws BPMException {
		if (log.isDebugEnabled()) {
			log.debug(
					"流程开始启动,PROCESSDEFINATIONKEY:{},PROCESSSTARTER;{},BIZID:{},PROCESSVARIABLES:{},BUSINESSCALLBACK:{}.",
					new Object[] { processDefinationKey, userId, bizId, processVariables, eventListener });
		}
		ProcessInstance processInstance = null;
		try {
			preCheck(processDefinationKey, userId, bizId);
			preSetup(userId, eventListener);
			if (processVariables == null) {
				processVariables = new HashMap<String, Object>();
			}
			processInstance = runtimeService.startProcessInstanceByKey(processDefinationKey, bizId, processVariables);
			...
			log.info("流程启动成功," + "流程定义KEY:{}," + "流程定义ID:{}," + "流程实例ID:{}," + "绑定业务ID:{}," + "启动人:{}," + "时间戳(ms):{}.",
					new Object[] { processInstance.getProcessDefinitionKey(), processInstance.getProcessInstanceId(),
							processInstance.getProcessInstanceId(), processInstance.getBusinessKey(), userId,
							System.currentTimeMillis() });
			return processInstance;
		} catch (Exception e) {
			throw new BPMException("BPM_EX_10", e.getMessage(), e);
	...
	@Override
	public ProcessInstance activateProcess(String processInstanceId, Map<String, Object> processVariables,
			BPMEventListener eventListener) throws BPMException {
		if (log.isDebugEnabled()) {
			log.debug(
					"激活流程,process instance id:{}.", new Object[] { processInstanceId });
		}
		ProcessInstance processInstance = null;
		try {
			BPMUtils.assertProcessInstanceIdNotNULL(processInstanceId);
			preSetup(eventListener);
			if (processVariables == null) {
				processVariables = new HashMap<String, Object>();
			}
			processInstance = extendService.activateProcessInstanceById(processInstanceId, processVariables);
			log.info("流程激活成功," + "流程定义KEY:{}," + "流程定义ID:{}," + "流程实例ID:{}," + "绑定业务ID:{}," + "时间戳(ms):{}.",
					new Object[] { processInstance.getProcessDefinitionKey(), processInstance.getProcessInstanceId(),
	...
	public void completeTask(String taskId, String userId, Map<String, Object> taskVaribles, BPMEventListener listener)
		...
		try {
			BPMUtils.assertTaskIdNotNULL(taskId);
	...
	@Override
	public void submitTask(DecisionModel decisionModel, BPMEventListener taskEventListener) throws BPMException {
		if (log.isDebugEnabled()) {
			log.debug("流程任务执行：TASKID:{},USERID:{}", new Object[] { decisionModel.getTaskId(), decisionModel.getUserId() });
		}
		try {
			BPMUtils.assertTaskIdNotNULL(decisionModel.getTaskId());
			preSetup(decisionModel.getUserId(), taskEventListener);
			Decision decisionInfo = decisionModel.convert();
			preSetup(decisionInfo);
			extendService.handleTaskSubmit(decisionInfo);
		} catch (Exception e) {
			throw new BPMException(e);
		} finally {
			postSetup();
	...
	@Override
	public void auditorRevocation(String processInstanceId, String executionId, String taskId, String userId,
			BPMEventListener listener) throws BPMException {
		if (log.isDebugEnabled()) {
		...
		try {
			...
			if (extendService != null) {
				extendService.auditorRevocation(processInstanceId, executionId, taskId);
			} else {
	...
	@Override
	public void fallbackTask(String processInstanceId, String currentTaskId, String destTaskId, String userId,
			Map<String, Object> taskVaribles) throws BPMException {
		if (log.isDebugEnabled()) {
			log.debug("任务退回操作：PROCESSINSTANCEID:{},currentTaskId:{}, destTaskId:{}, USERID:{}", new Object[] 
					{ processInstanceId, currentTaskId, destTaskId, userId });
		}
		try {
			BPMUtils.assertProcessInstanceIdNotNULL(processInstanceId);
			BPMUtils.assertTaskIdNotNULL(currentTaskId);
			BPMUtils.assertTaskIdNotNULL(destTaskId);
			preSetup(userId, null);
			if (extendService != null) {
				extendService.fallbackTask(processInstanceId, currentTaskId, destTaskId);
			} else {
				throw new RuntimeException("当前流程组件暂不支持");
			}
		} catch (Exception e) {
	...
	@Override
	public ExtendService getExtendService() {
