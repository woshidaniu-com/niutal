...
import com.woshidaniu.workflow.constant.WorkFlowConstant;
import com.woshidaniu.workflow.enumobject.BusinessEnum;
import com.woshidaniu.workflow.enumobject.MessageTypeEnum;
...
public class SpWorkFlowServiceImpl extends BaseInterfaceServiceImpl implements ISpWorkFlowService {
	...
	public MessageOutputProvider smsOutputProvider;
	...
	public IXxdlService xxdlService;
	...
	public BaseResult addSpWorkFlow(HashMap<String, String> map) throws WorkFlowException {
		...
		try {
			...
			String sjfwztj = map.get("sjfwztj");
			String sjfwztjQz = map.get("sjfwztjQz");
			String sendMes = map.get("sendMes");
			String sendMail = map.get("sendMail");
			String sendSms = map.get("sendSms");
			String contentMes = map.get("contentMes");
			String contentMail = map.get("contentMail");
			String contentSms = map.get("contentSms");
			String contentMesEnd = map.get("contentMesEnd");
			String contentMailEnd = map.get("contentMailEnd");
			String contentSmsEnd = map.get("contentSmsEnd");
			String sqrYhm = map.get("sqrYhm");
			String sqrJsdm = map.get("sqrJsdm");
			
			if (StringUtil.isEmpty(businessCode) && StringUtil.isEmpty(procedureId)) {
			...
			if(StringUtil.isNotEmpty(businessCode) && StringUtil.isEmpty(procedureId)){
				spProcedure = spProcedureService.findSpProcedureByBCode(businessCode);
			}else if(StringUtil.isEmpty(businessCode) && StringUtil.isNotEmpty(procedureId)){
				spProcedure = spProcedureService.findSpProcedureByPid(procedureId, true);
			}else if(StringUtil.isNotEmpty(businessCode) && StringUtil.isNotEmpty(procedureId)){
				spProcedure = spProcedureService.findSpProcedureByPid(procedureId, true);
			}
			if (spProcedure == null) {
			...
			if (spNodeList != null && spNodeList.size() > 0) {
				for (SpNode spNode : spNodeList) {
					if (spNode != null) {
						...
						if(BlankUtils.isBlank(spWorkNode.getSjfwztj())) spWorkNode.setSjfwztj(sjfwztj);
						if(BlankUtils.isBlank(spWorkNode.getSjfwztj_qz())) spWorkNode.setSjfwztj_qz(sjfwztjQz);
						
						if(BlankUtils.isBlank(spWorkNode.getSend_mes())) spWorkNode.setSend_mes(sendMes);
						if(BlankUtils.isBlank(spWorkNode.getSend_mail())) spWorkNode.setSend_mail(sendMail);
						if(BlankUtils.isBlank(spWorkNode.getSend_sms())) spWorkNode.setSend_sms(sendSms);
						
						if(BlankUtils.isBlank(spWorkNode.getContent_mes())) spWorkNode.setContent_mes(contentMes);
						if(BlankUtils.isBlank(spWorkNode.getContent_mail())) spWorkNode.setContent_mail(contentMail);
						if(BlankUtils.isBlank(spWorkNode.getContent_sms())) spWorkNode.setContent_sms(contentSms);
						
						if(BlankUtils.isBlank(spWorkNode.getContent_mes_end())) spWorkNode.setContent_mes_end(contentMesEnd);
						if(BlankUtils.isBlank(spWorkNode.getContent_mail_end())) spWorkNode.setContent_mail_end(contentMailEnd);
						if(BlankUtils.isBlank(spWorkNode.getContent_sms_end())) spWorkNode.setContent_sms_end(contentSmsEnd);
						
						spWorkNode.setSqr_yhm(sqrYhm);
						spWorkNode.setSqr_jsdm(sqrJsdm);
...
//				|| null != taskList || taskList.size() > 0
				...
				if (nodeList != null && nodeList.size() > 0) {
					...
					//由申请人发起的消息删除
					this.cancelXxdlBySqr(spWorkNode);
					return true;
				}
			} else {
	...
	public NodeListSqlResult queryWorkFlowListSql(WorkAuditingQuery query) throws WorkFlowException {
		...
		try {
			...
			if (query != null && query.getVersion() == 1) {
				...
				if (BlankUtil.isBlank(status)||status.length==0) {
					//则默认查全部
			...
			//新版本走更好的路
				...
				//查询待处理数据
				if(StringUtil.equals(status[0], WorkNodeEStatusEnum.WAIT_EXECUTE.getKey())){
				...
				//查询已处理数据
				}else if(StringUtil.equals(status[0], WorkNodeEStatusEnum.ALREADY_EXECUTE.getKey())){
	...
	private boolean isExists(String[] roles, String role){
		boolean bol = true;
		if(roles != null && roles.length > 0 && StringUtil.isNotEmpty(role)){
			...
			if (spWorkNode != null) {
				...
				*/
				...
				if (StringUtil.isEmpty(spWorkNode.getRoleId()) 
						|| StringUtil.isEmpty(spWorkNode.getUserId())) {
					...
					if(newNode != null){
						if(StringUtil.isEmpty(newNode.getRoleId()) && StringUtil.isEmpty(newNode.getUserId())){
				...
				if (spWorkNode.getStatus().equals(// 保存
					...
					spWorkNode.setEstatus(WorkNodeEStatusEnum.ALREADY_EXECUTE.getKey());
					this.saveSpWorkNode(spWorkNode, true, false);
					 ...
					 */
					spWorkNode.setEstatus(WorkNodeEStatusEnum.ALREADY_EXECUTE.getKey());
					spWorkNode.setMessageType(MessageTypeEnum.AUDITTED_PERSON.getKey());
					...
					// 如果是退回申报人则删除整个工作审批实例
						...
						if (StringUtil.isNotEmpty(spWorkNode.getWid())) {
							// 记录操作日志
							spWorkNodeService.insertNodeLog(spWorkNode);
							//给被退回的消息人发送消息(因为发送消息要依赖SP_WORK_NODE表中的数据，所以这里先发消息再执行后面的删除操作)
							spWorkNode.setMessageType(MessageTypeEnum.AUDITTED_PERSON.getKey());
							this.addXxdl(spWorkNode);
							//删除审批流工作实例
							...
							//更新教务中队列的信息(如果发送给了一批人的消息，其中一个人已经做审核操作，则把其它人收到的消息状态更新为：已处理)
							this.updateXxdl(spWorkNode);
...
//					}
					...
					do {						
						...
						if (StringUtil.isEmpty(uNodeId))
							continue;
						spWorkNode.setNodeId(uNodeId);
						
						//如果当前需要执行节点id等于返回节点id，则发送消息，否则不发
						if(returnNodeId.equalsIgnoreCase(uNodeId)){
							SpWorkNode cNode = this.queryWorkNodeByWidAndNodeId(spWorkNode.getWid(), spWorkNode.getNodeId());
							if(!BlankUtils.isBlank(cNode)){
								if(!BlankUtils.isBlank(cNode.getRoleId())) spWorkNode.setRoleId(cNode.getRoleId());
					        	if(!BlankUtils.isBlank(cNode.getSjfwztj())) spWorkNode.setSjfwztj(cNode.getSjfwztj());
					    		if(!BlankUtils.isBlank(cNode.getSjfwztj_qz())) spWorkNode.setSjfwztj_qz(cNode.getSjfwztj_qz());
					    		
					    		if(!BlankUtils.isBlank(cNode.getSend_mes())) spWorkNode.setSend_mes(cNode.getSend_mes());
					    		if(!BlankUtils.isBlank(cNode.getSend_mail())) spWorkNode.setSend_mail(cNode.getSend_mail());
					    		if(!BlankUtils.isBlank(cNode.getSend_sms())) spWorkNode.setSend_sms(cNode.getSend_sms());
					    		
					    		if(!BlankUtils.isBlank(cNode.getContent_mes())) spWorkNode.setContent_mes(cNode.getContent_mes());
					    		if(!BlankUtils.isBlank(cNode.getContent_mail())) spWorkNode.setContent_mail(cNode.getContent_mail());
					    		if(!BlankUtils.isBlank(cNode.getContent_sms())) spWorkNode.setContent_sms(cNode.getContent_sms());
					    		
					    		if(!BlankUtils.isBlank(cNode.getContent_mes_end())) spWorkNode.setContent_mes_end(cNode.getContent_mes_end());
					    		if(!BlankUtils.isBlank(cNode.getContent_mail_end())) spWorkNode.setContent_mail_end(cNode.getContent_mail_end());
					    		if(!BlankUtils.isBlank(cNode.getContent_sms_end())) spWorkNode.setContent_sms_end(cNode.getContent_sms_end());
					    		
					    		if(!BlankUtils.isBlank(cNode.getSqr_yhm())) spWorkNode.setSqr_yhm(cNode.getSqr_yhm());
					    		if(!BlankUtils.isBlank(cNode.getSqr_jsdm())) spWorkNode.setSqr_jsdm(cNode.getSqr_jsdm());
					        }
	...
	public BaseResult doCancelAuditingRsult(SpWorkNode spWorkNode, String[] roleIdArray, String[] userIdArray)
		...
		try {
			...
			if(spWorkLineList != null && spWorkLineList.size() > 0){
				...
				this.saveSpWorkNode(downSpWrokNode, false, false);
				//撤销删除消息队列
				downSpWrokNode.setNodeType(NodeTypeEnum.NORMAL_NODE.getKey());
				this.cancelXxdl(downSpWrokNode);
			}else{
				// 如果是最后一个节点
				SpWorkNode spWorkNode2 = spWorkNodeService.findWorkNodeByWidAndNodeId(spWorkNode);
				if(spWorkNode2 != null){
					...
					//撤销删除消息队列
					SpWorkNode cancelWorkNode = new SpWorkNode();
					cancelWorkNode.setWid(spWorkNode.getWid());
					cancelWorkNode.setPid(spWorkNode.getPid());
					cancelWorkNode.setNodeId(spWorkNode.getNodeId());
					cancelWorkNode.setMessageType(MessageTypeEnum.AUDITTED_PERSON.getKey());
					this.cancelXxdl(cancelWorkNode);
	...
	private void changeNextNodeStatus(SpWorkNode spWorkNode) {
		...
		if (null != spWorkNode && null != spWorkNode.getNodeType()) {
			if(!NodeTypeEnum.END_NODE.getKey().equalsIgnoreCase(spWorkNode.getNodeType())){
				SpWorkLine spWorkLine = new SpWorkLine();
				spWorkLine.setWid(spWorkNode.getWid());
				spWorkLine.setUnodeId(spWorkNode.getNodeId());
				List<SpWorkLine> spWorkLineList = spWorkLineService.findWorkLineList(spWorkLine);
	...
	public boolean addXxdl(SpWorkNode spWorkNode) throws WorkFlowException {
		try {
			if (spWorkNode != null) {
				String sendMes = spWorkNode.getSend_mes();
				String sendMail = spWorkNode.getSend_mail();
				String sendSms = spWorkNode.getSend_sms();
				String contentMes = spWorkNode.getContent_mes();
				String contentMail = spWorkNode.getContent_mail();
				String contentSms = spWorkNode.getContent_sms();
				
				//判断是要发送站内
				XxdlModel xxdlModel = new XxdlModel();
				//根据pid获取跳转路径
				xxdlModel.setJsdm(spWorkNode.getRoleId());
				xxdlModel.setSjfwztj(spWorkNode.getSjfwztj());
				xxdlModel.setSjfwztj_qz(spWorkNode.getSjfwztj_qz());
				xxdlModel.setpId(spWorkNode.getPid());
				xxdlModel.setwId(spWorkNode.getWid());
				xxdlModel.setNodeId(spWorkNode.getNodeId());
				xxdlModel.setLx(MessageTypeEnum.AUDITTING_ROLE.getKey());
				//如果是结束节点，需要发给申请人
				if (null != spWorkNode.getMessageType()&&MessageTypeEnum.AUDITTED_PERSON.getKey().equalsIgnoreCase(spWorkNode.getMessageType())){
					contentMes = spWorkNode.getContent_mes_end();
					contentMail = spWorkNode.getContent_mail_end();
					contentSms = spWorkNode.getContent_sms_end();
					xxdlModel.setJsdm(spWorkNode.getSqr_jsdm());
					xxdlModel.setLx(MessageTypeEnum.AUDITTED_PERSON.getKey());
				}
				xxdlModel.setXxbt(contentMes);
				xxdlModel.setXxnr(contentMes);
				
				xxdlService.addXxdl(xxdlModel);
				
				//判断是要发送邮件
				if(!BlankUtils.isBlank(sendMail)&&WorkFlowConstant.STATUS_YES.equals(sendMail)){ 
					//TODO 
				}
				
				//判断是要发送短信
				if(!BlankUtils.isBlank(sendSms)&&WorkFlowConstant.STATUS_YES.equals(sendSms)&&!BlankUtils.isBlank(contentMes)){  
					List<XxdlModel> xxdlList = xxdlService.selectXxdlxxByxxzH(xxdlModel);
					if(!BlankUtils.isBlank(xxdlList)&&xxdlList.size()>0){
						for(XxdlModel cycleXxdlModel:xxdlList){
							if(!BlankUtils.isBlank(xxdlModel)&&!BlankUtils.isBlank(cycleXxdlModel.getSjhm())){
								//构造消息体
								MessageBody message = new MessageBody(MessageType.SMS, String.valueOf(sequence.nextId()) );
								// 接收人的手机号码
								message.getBody().put(MessageKey.SMS_SENDTO, cycleXxdlModel.getSjhm());

								// 短信的内容体
								//String content = PatternFormatUtils.format("", contentSms);
								message.getBody().put(MessageKey.SMS_CONTENT, cycleXxdlModel.getXxnr_sms());
										
								boolean result = getSmsOutputProvider().output(message);
								//判断邮件是否未发送成功
								if(!result){
									//逻辑代码
								}
	...
	public boolean cancelXxdl(SpWorkNode spWorkNode) throws WorkFlowException {
		try {
			if (spWorkNode != null) {
				XxdlModel xxdlInfo = new XxdlModel();
				xxdlInfo.setwId(spWorkNode.getWid());
				//撤销发送到申请人的消息
				if(MessageTypeEnum.AUDITTED_PERSON.getKey().equals(spWorkNode.getMessageType())){
					xxdlService.deleteSqrXxdl(xxdlInfo);
				}else{
					xxdlInfo.setpId(spWorkNode.getPid());
					xxdlInfo.setNodeId(spWorkNode.getNodeId());
					xxdlService.deleteXxdl(xxdlInfo);
				}
			}
		} catch (Exception e) {
			log.error(e.getMessage());
	...
	public boolean cancelXxdlBySqr(SpWorkNode spWorkNode) throws WorkFlowException {
		try {
			if (spWorkNode != null && !BlankUtils.isBlank(spWorkNode.getWid())) {
				XxdlModel xxdlInfo = new XxdlModel();
				xxdlInfo.setwId(spWorkNode.getWid());
				xxdlService.deleteXxdlBySqr(xxdlInfo);
			}
		} catch (Exception e) {
			log.error(e.getMessage());
			throw new WorkFlowException(e.getMessage(), e);
	...
	public boolean updateXxdl(SpWorkNode spWorkNode) throws WorkFlowException {
		try {
			...
			xxdlModel.setwId(spWorkNode.getWid());
			xxdlModel.setNodeId(spWorkNode.getNodeId());
			xxdlService.updateXxdlclzt(xxdlModel);
		} catch (Exception e) {
			log.error(e.getMessage());
			throw new WorkFlowException(e.getMessage(), e);
		}
	...
	public IXxdlService getXxdlService() {
		return xxdlService;
	}

	public void setXxdlService(IXxdlService xxdlService) {
		this.xxdlService = xxdlService;
	}
