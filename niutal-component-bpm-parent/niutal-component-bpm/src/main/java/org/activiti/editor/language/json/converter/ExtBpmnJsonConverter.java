 ...
 * <p>
 * <h3>niutal框架
 * <h3>说明：扩展BpmnJsonConverter
 * <p>
 * 
 * @className:org.activiti.editor.language.json.converter.ExtBpmnJsonConverter.java
 * @author <a href="mailto:337836629@qq.com">zhidong.kang[1036]<a>
 * @version 2017年4月1日上午10:56:11
 */
public class ExtBpmnJsonConverter implements EditorJsonConstants, StencilConstants, ActivityProcessor {
	protected static final Logger LOGGER = LoggerFactory.getLogger(ExtBpmnJsonConverter.class);

	final String PROPERTY_PROCESS_NAMESPACE_EXT = "processnamespace";
	...
	static {
		...
		// connectors
		ExtSequenceFlowJsonConverter.fillTypes(convertersToBpmnMap, convertersToJsonMap);
		...
		ExtUserTaskJsonConverter.fillTypes(convertersToBpmnMap, convertersToJsonMap);
	...
	public ObjectNode convertToJson(BpmnModel model) {
		...
		if (StringUtils.isNoneEmpty(model.getTargetNamespace())) {
			propertiesNode.put(PROPERTY_PROCESS_NAMESPACE_EXT, model.getTargetNamespace());
			ObjectNode namespaceNode = objectMapper.createObjectNode();
			namespaceNode.put("id", model.getTargetNamespace());
			propertiesNode.put(PROPERTY_PROCESS_NAMESPACE_EXT, namespaceNode);
		}
	...
	protected void processFlowElement(FlowElement flowElement, FlowElementsContainer container, BpmnModel model,
		...
		if (converter != null) {
			try {
				converter.newInstance().convertToJson(flowElement, this, model, container, shapesArrayNode, containerX,
						containerY);
			} catch (Exception e) {
	...
	protected void processArtifacts(FlowElementsContainer container, BpmnModel model, ArrayNode shapesArrayNode,
		...
		for (Artifact artifact : container.getArtifacts()) {
			...
			if (converter != null) {
				try {
					converter.newInstance().convertToJson(artifact, this, model, container, shapesArrayNode, containerX,
							containerY);
				} catch (Exception e) {
	...
	public BpmnModel convertToBpmnModel(JsonNode modelNode) {
		...
		if (nonEmptyPoolFound == false) {
			...
			//String namespace = BpmnJsonConverterUtil.getPropertyValueAsString(PROPERTY_PROCESS_NAMESPACE_EXT, modelNode);
			JsonNode namespace = BpmnJsonConverterUtil.getProperty(PROPERTY_PROCESS_NAMESPACE_EXT, modelNode);
			if (namespace!=null && (namespace.get("id")!=null)) {
				bpmnModel.setTargetNamespace(namespace.get("id").asText());
			}
