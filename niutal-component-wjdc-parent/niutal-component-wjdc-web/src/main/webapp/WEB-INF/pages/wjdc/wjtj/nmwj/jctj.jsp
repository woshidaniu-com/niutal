		...
		<script type="text/javascript">
			...
			function searchResult() {
				...
				if(jQuery("#rightContent").length > 0){
					ajaxSubForm("form_djtj", "<%=systemPath %>/zfxg/wjdc/wjtj_jctjNmwj.html");
				}else{
					subForm("<%=systemPath %>/zfxg/wjdc/wjtj_jctjNmwj.html");
				}				
			}
		</script>
	</head>

	<s:form namespace="/zfxg/wjdc" action="wjtj_jctjNmwj" method="post" id="form_djtj" theme="simple">
			...
			<div class="comp_title">
		      <ul class="cla_lxbt">
				<s:iterator value="lxbtList" >
					 <li id="${lxid}"><a href="#" onclick="refRightContent('<%=systemPath %>/zfxg/wjdc/wjtj_jctjNmwj.html?lxid=${lxid }'+'&wjid=${wjid }');"><span>${lxmc}</span></a></li>
				</s:iterator>
		      </ul>
		    </div>
